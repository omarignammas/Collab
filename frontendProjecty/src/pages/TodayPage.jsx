      ...
      {loading ? (
        ...
        <div className="space-y-7">
          ...
          <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <Card className="border-border/80 bg-card"><CardContent className="p-5"><div className="flex items-start gap-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><Flame className="h-5 w-5" /></span><div className="min-w-0"><p className="font-semibold text-foreground">Momentum comes from real work</p><p className="mt-1 text-sm text-muted-foreground">{momentum?.completedTasks || 0} tasks completed, {momentum?.focusMinutes || 0} focused minutes, and {momentum?.activeDays || 0} active days this week.</p><div className="mt-4 grid grid-cols-4 gap-2 text-center text-xs"><div className="rounded-lg bg-muted/60 px-2 py-2"><b className="block text-foreground">{momentum?.taskPoints || 0}/50</b><span className="text-muted-foreground">tasks</span></div><div className="rounded-lg bg-muted/60 px-2 py-2"><b className="block text-foreground">{momentum?.focusPoints || 0}/30</b><span className="text-muted-foreground">focus</span></div><div className="rounded-lg bg-muted/60 px-2 py-2"><b className="block text-foreground">{momentum?.quizPoints || 0}/10</b><span className="text-muted-foreground">quiz</span></div><div className="rounded-lg bg-muted/60 px-2 py-2"><b className="block text-foreground">{momentum?.consistencyPoints || 0}/10</b><span className="text-muted-foreground">rhythm</span></div></div></div></div></CardContent></Card>
            <Card className="border-border/80 bg-card"><CardContent className="flex h-full items-center gap-4 p-5"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[hsl(var(--chart-3))]/15 text-[hsl(var(--chart-3))]"><UsersRound className="h-5 w-5" /></span><div className="min-w-0 flex-1">{activeCircle ? <><p className="font-semibold text-foreground">{activeCircle.name}</p><p className="mt-1 text-sm text-muted-foreground">{activeCircle.activeMemberCount} people · {activeCircle.collectiveMomentum}% collective momentum this week.</p></> : <><p className="font-semibold text-foreground">Progress is better shared</p><p className="mt-1 text-sm text-muted-foreground">Create a private Circle with friends who want to show up together.</p></>}</div><Button type="button" size="sm" variant="outline" onClick={() => navigate('/circles')}>{activeCircle ? 'Open' : 'Create'}</Button></CardContent></Card>
          </section>
        </div>
      )}
